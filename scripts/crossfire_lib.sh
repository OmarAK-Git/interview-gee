#!/usr/bin/env bash
# Shared Crossfire helpers. No source-time real-home kill. No default HERMES_HOME.
# Demo/tests source demo_common.sh (landmine). Practice sources practice_common.sh
# (one-path Monday allowlist).
set -euo pipefail

crossfire_lib_dir=$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]:-$0}")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$crossfire_lib_dir/.." && pwd)
